...
export function Combobox(props: ComboBoxProps) {
  ...
  const formatParentUnits = (units: any[]) => {
    if (!units || units.length === 0) return "";

    if (units.length <= 2) {
      return units.map((u) => u.label).join(" / ");
    }
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col">
                          {option.parentStructureUnits &&
                            option.parentStructureUnits.length > 0 && (
                              <p className="text-sm font-normal text-[#71717A]">
                                {formatParentUnits(option.parentStructureUnits)}
                              </p>
                            )}
